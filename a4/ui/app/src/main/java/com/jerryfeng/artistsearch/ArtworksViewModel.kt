...
class ArtworksViewModel : ViewModel() {
    ...
    private val _data = MutableStateFlow<List<Artworks>>(emptyList())
    val data: StateFlow<List<Artworks>> = _data
            ...
            _artistId.collect { artistId ->
                try {
                    if (artistId != "") {
                        ...
                        _data.value = RetrofitClient.apiService.getArtworks(artistId)
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                } finally {
